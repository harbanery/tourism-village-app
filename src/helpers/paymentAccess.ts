...
const key = (orderId: string) => `tourism-village:paymentAccess:${orderId}`;

/** Terbitkan tiket akses untuk order (dipanggil sebelum navigasi). */
export function issuePaymentAccess(orderId: string): void {
  if (typeof window === "undefined") return;
  window.sessionStorage.setItem(key(orderId), "1");
}

/** Cek keberadaan tiket TANPA menghapus (aman dipanggil berulang). */
export function peekPaymentAccess(orderId: string): boolean {
...
export function consumePaymentAccess(orderId: string): void {
  if (typeof window === "undefined") return;
